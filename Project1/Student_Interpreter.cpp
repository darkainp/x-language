...
//?????????????????????????????
Fix all code functions that have //???????????????????? by them
//?????????????????????????????
...
unsigned int S;		//value of top of the stack
unsigned int L;		//Local, points to the top of the system stack (current scope's variableS)
unsigned int G;		//global, points to the beginning of the statics
/**************************************************************/
...
/**
* T before the function is number of elements in the stack
* @return the value at top of stack X
*/
unsigned int pop(void) {
	int value = -1;
	if (T) {
...
/*
* Push a value into the EXPRESSION stack
*
* Only if T < StackSize: the stack is not over flow
* @param value: integer value to push into the stack
*/
void push(unsigned int value) {
	if (T < StackSize) {
		X[T++] = value;
...
/*Display all the data in the stack from top of the stack to bottom of the stack
* Not actually deleting stack, just showing it
*/
void DumpStack()
{
	...
	cout << " End stack " << endl;
	cin >> tach; //to get any char before moving on lol
}
/***************************************************/
...
/*Getting the value in addr and return it
* After this function, addr is inc by 1
*
* @param addr: the address to get the value from
* @return the value in addr*/
unsigned int get1(unsigned int& addr) {
	return M[addr++ & AddrMask]; //wtf
}

/*Putting a 4 bytes value in Memory starting from addr
*
* @param addr: the address to put the the value into
* @param value: value to put in the address
*/
void put4(unsigned int& addr, unsigned int value) {

	/*since each addr can only hold 1byte,
	this function will put in the first 16 bits of the value*/
	put2(addr, value);
	put2(addr, value >> 16); //put in the last 16 bits of the value
}

/*Putting a 2 bytes value in Memory starting from addr
*
* @param addr: the address to put the the value into
* @param value: value to put in the address
*/
void put2(unsigned int& addr, unsigned int value) {
	put1(addr, value);
	put1(addr, value >> 8); //put in the last 8 bits of the value
}

/*Putting a 1 bytes value in Memory starting from addr
*
* @param addr: the address to put the the value into
* @param value: value to put in the address
*/
	...
	while (!loaded) {
		...
		/*    strcpy( fname, commandLine);
		  ...
		  */  ifstream fin;
		  ...
		  if (!fin || fname.length() > MaxCommandLineLen) {
			  cout << "*** ERROR: Invalid Filename" << endl;
		  }
		  else {
			  for (int i = 0; i < fname.length(); i++)
				  commandLine[i] = fname[i];
...
/**
* Fetching 4 bytes
* @return 4 bytes of data in memory starting from the PC
*/
unsigned int next4(void) {
	int a = next2();
	int b = next2() << 16;
...
/**********************************************************
* Fetching 2 bytes
* @return 2 bytes of data in memory starting from the PC
***********************************************************/
unsigned int next2(void) {
	int a = next1();
	int b = next1() << 8;
...
/**
*  Fetching 1 bytes
* @return 1 bytes of data in memory starting from the PC
*/
unsigned int next1(void) {
	return get1(PC);
}
	...
	while (!done) {
		...
		while (!done) {
			...
			*/	testdata << "\tT:" << T;
			...
			if (DEBUG)
				cout << "PC: " << hex << IR << " M[" << dec << count << "]" << endl;
			...
			case ADD:
				//?????????????????????????????		
				b = pop(); //assign the value from top of stack to b
				push(pop() + b);	//cal the sum of the 1st 2 value in the stack and put it back in the stack
				break;
			case SUB:
				b = pop();
				push(pop() - b);
			...
			case COPY: //MAKE COPY OF TOP OF THE STACK
				b = pop();
				push(b);
				push(b);
				break;
			...
			case DSTK: /* Grab top stack val and display it to the screen*/
			{cout << "Stack: ";
			cout << hex;
			...
			default: /* No instruction match up with op code*/
				cout << "Invalid Instruction. PC:" << dec << PC << endl;
				done = true;
				break;
			}/* End switch*/
		} /*End while(!done)*/
